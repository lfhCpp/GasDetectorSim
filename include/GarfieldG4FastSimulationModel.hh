...
class GarfieldG4FastSimulationModel : public G4VFastSimulationModel {
 public:
  ...
  void SetPhysics(GarfieldPhysics* fGarfieldPhysics);      //未实现
  void WriteGeometryToGDML(G4VPhysicalVolume* physicalVolume);

  virtual G4bool IsApplicable(const G4ParticleDefinition&);//判断粒子在Grafield中是否可用
  virtual G4bool ModelTrigger(const G4FastTrack&);         //模式触发，确定模拟程序
  virtual void DoIt(const G4FastTrack&, G4FastStep&);      //Garfield模拟
